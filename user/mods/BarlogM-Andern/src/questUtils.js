...
function cheeseQuests(container) {
    ...
    Object.entries(tables.templates.quests).forEach(([questId, quest]) => {
        const questConditions = quest.conditions.AvailableForFinish.filter((questCondition) => questCondition.conditionType === "CounterCreator");
        if (questConditions !== undefined) {
            questConditions.forEach((questCondition) => {
                const questConditionCounters = questCondition.counter?.conditions.filter((conditionCounter) => (conditionCounter.conditionType === "Kills") || (conditionCounter.conditionType === "Shots") || (conditionCounter.conditionType === "Equipment"));
                if (questConditionCounters !== undefined) {
                    const questConditionCountersToDelete = [];
                    ...
                    if (questConditionCountersToDelete.length > 0) {
                        questCondition.counter.conditions = questCondition.counter.conditions.filter(c => !questConditionCountersToDelete.includes(c));
                    }
                }
            });
        }
...
exports.default = cheeseQuests;
function addAllDmr(conditionCounter) {
...
function replaceWithAnySilencedWeapon(conditionCounter) {
...
function replaceWithBoltyAndDMRSilencers(conditionCounter) {
    conditionCounter.weaponModsInclusive = [
        ["5b86a0e586f7745b600ccb23"],
        ["59bffbb386f77435b379b9c2"],
        ["593d489686f7745c6255d58a"],
