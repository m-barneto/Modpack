        ...
        setTimeout(() => {
            ...
            itemsToUpdate.forEach(({ id, name_gun }) => {
                ...
                if (item) {
                    ...
                    logger.logWithColor(`${name_gun} item ID found. Сhanges applied.`, LogTextColor_1.LogTextColor.GREEN);
                }
                else {
                    // Log message if the item ID is not found
                    logger.logWithColor(`${name_gun} item ID not found. Changes not applied.`, LogTextColor_1.LogTextColor.RED);
                }
            });
            ammoToUpdate.forEach(({ id, heatFactor, name_ammo }) => {
                ...
                if (ammoItem) {
                    ...
                    logger.logWithColor(`${name_ammo} item ID found. Changes applied.`, LogTextColor_1.LogTextColor.GREEN);
                }
                else {
                    // Log message if the ammo ID is not found
                    logger.logWithColor(`${name_ammo} item ID not found. Changes not applied.`, LogTextColor_1.LogTextColor.RED);
                }
            });
